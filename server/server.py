...
async def registers(request):
    ...
    server.agents = data
    return web.json_response({'code': 0, 'msg': 'registered successfully!', 'data': {'url': cfg.getInflux('url'),
                              'org': cfg.getInflux('org'), 'token': cfg.getInflux('token'),
                              'bucket': cfg.getInflux('bucket')}})
